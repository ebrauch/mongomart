...
function ItemDAO(database) {
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        // this.db.collection('item').find({$text: {$search: query}}).forEach(function(doc) {
        //   numItems++;
        // }, function(err) {
        //   callback(numItems);
        // });
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        // this.db.collection('item').findOne({'_id': itemId}, function(err, doc) {
          ...
          // dbRef.collection('item').update({'_id': itemId}, doc);
          // callback(doc);

          this.db.collection('item').findAndModify({'_id': itemId}, {'_id': 1}, {$addToSet: {'reviews': reviewDoc}}, {new: true}, function(err, doc) {
            if (err) {
              console.log(err);
              return;
            }
